...
def fetch_current_weather_for_system(db, system, session):
    """获取单个系统的实时天气数据"""
    try:
        fetch_and_store_current(db, system, session=session)
...
def main():
    ...
    try:
        ...
        for system in systems:
            if fetch_current_weather_for_system(db, system, session):
                success_count += 1
